...
// ConfigureAcceleratorsForCaffe2JobSpec adds any accelerator specific configuration to the pods.
func ConfigureAcceleratorsForCaffe2JobSpec(c *api.Caffe2JobSpec, accelerators map[string]api.AcceleratorConfig) error {
	for _, r := range c.ReplicaSpecs {
		...
		for i, c := range r.Template.Spec.Containers {
			if c.Name == string(api.CAFFE2) {
				...
				a := map[string]api.AcceleratorConfig{}
